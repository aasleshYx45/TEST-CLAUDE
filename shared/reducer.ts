...
export function reducer(state: AppState, action: Action): AppState {
  ...
  switch (action.type) {
    ...
    // Restoring what was already saved is not a change, so it keeps the stored
    // timestamp rather than reporting the app as just updated.
    case 'state/hydrate':
      return { ...emptyState(), ...action.state, version: VERSION }
...
/** Fills in anything a stored or imported blob is missing, so a partial or
    older payload can never leave the app in a half-initialised state. */
export function hydrate(parsed: Partial<AppState> | null | undefined): AppState {
